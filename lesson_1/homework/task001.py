...
# он может от него удаляться. Во второй строке содержится два целых числа Q и M — аналогичные данные для Маши. Все числа
...
def count_painted_trees(p, v, q, m):
    return 2 * (v + m + 1) - max(0, min(p + v, q + m) - max(p - v, q - m) + 1)


p, v = map(int, input().split())
q, m = map(int, input().split())
print(count_painted_trees(p, v, q, m))

# трансформированный код с С++
# Ок
