...
class MyData:
    def __init__(self, dev=0, cost=0, load=0):
        self.dev = dev
        self.cost = cost
        self.load = load
...
if __name__ == "__main__":
    ...
    part_in_device = {i: set() for i in range(a)}
    device_in_part = {i: set() for i in range(b)}
    cost = {}

    res = [0] * a
    otvet = [0] * a
    target_part_for_device = [-1] * a
    device_for_download_part = [-1] * a
    ...
    while True:
        ...
        count_device_have_part = sorted([(i, len(device_in_part[i])) for i in range(b)], key=lambda x: (x[1], x[0]))

        for i in range(1, a):
            for item in count_device_have_part:
                if item[0] not in part_in_device[i]:
                    target_part_for_device[i] = item[0]
                    break
        ...
        for i in range(1, a):
            if target_part_for_device[i] != -1:
                min_parts = 9999
                dev = -1
                for item in device_in_part[target_part_for_device[i]]:
                    if min_parts > len(part_in_device[item]):
                        min_parts = len(part_in_device[item])
                        dev = item
                device_for_download_part[i] = dev
            else:
                device_for_download_part[i] = -1

        tmp_cost = set()

        for i in range(a):
            tmp = [j for j in range(1, a) if device_for_download_part[j] == i]

            mas = []
            for item in tmp:
                a = MyData(dev=item, cost=cost.get((i, item, 0)), load=len(part_in_device[item]))
                mas.append(a)
            ...
            if mas:
                a = mas[0]
                part_in_device[a.dev].add(target_part_for_device[a.dev])
                device_in_part[target_part_for_device[a.dev]].add(a.dev)
                tmp_cost.add((a.dev, i))
                res[a.dev] += 1

                if res[a.dev] == b:
                    otvet[a.dev] = kk

        for item in tmp_cost:
            cost[item] = cost.get(item, 0) + 1

        if all(res[h] == b for h in range(1, a)):
            break
